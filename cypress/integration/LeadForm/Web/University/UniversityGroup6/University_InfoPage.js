    describe('University Info Page- Sanity', () => {
        beforeEach(() => {
            ...
            cy.fixture('endpoint').then((data)=>{
                if(Cypress.env("varEnv")=='prod'){
                    ...
                    cy.visit(Cypress.env("prod")+data.university_info)
                }else{
                    cy.log(Cypress.env("varEnv"))
                    cy.visit(Cypress.env("stage")+data.university_info)
                }
            })
            */
           cy.visit("https://collegedunia.com/university/25602-iim-bangalore-indian-institute-of-management-bangalore");
        })
        ...
        it('Btech Form- Check detailed fees', () => {
            cy.fixture('xpath').then((data)=>{
                cy.waitUntil(() => cy.get(data.col_checkdetailedfee_btn).then($el => $el.trigger('click')))
                cy.LeadForm('web_india_btech')
            })
        })
        ...
        it('Btech Form- Download brochure(PGPM)', () => {
            cy.fixture('xpath').then((data)=>{
                cy.waitUntil(() => cy.get(data.col_db_course).then($el => $el.trigger('click')))
                cy.LeadForm('web_india_btech')
            })
        })
