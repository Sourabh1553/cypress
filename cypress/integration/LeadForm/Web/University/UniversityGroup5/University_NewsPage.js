    describe('University News Page- Sanity', () => {
        beforeEach(() => {
            ...
            cy.fixture('endpoint').then((data)=>{
                if(Cypress.env("varEnv")=='prod'){
                    ...
                    cy.visit(Cypress.env("prod")+data.university_news)
                }else{
                    cy.log(Cypress.env("varEnv"))
                    cy.visit(Cypress.env("stage")+data.university_news)
                }
            })
        })
        ...
        it('Btech- College news and article', () =>{
            cy.fixture('xpath').then((data)=>{
                cy.get(data.col_colnews_article).click()
                cy.LeadForm('web_india_btech')
            })
        })

        it('Btech- University news and article', () =>{
            cy.fixture('xpath').then((data)=>{
                cy.get(data.uni_uninews_article).click()
                cy.LeadForm('web_india_btech')
            })
        })
        
        it('Btech- Exam news and article', () =>{
            cy.fixture('xpath').then((data)=>{
                cy.get(data.col_examnews_article).click()
                cy.LeadForm('web_india_btech')
            })
        })
