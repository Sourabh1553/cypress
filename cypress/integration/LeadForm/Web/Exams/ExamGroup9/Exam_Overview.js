    describe('Exam Overview Page- Sanity', () => {
        beforeEach(() => {
            ...
            cy.fixture('endpoint').then((data)=>{
                if(Cypress.env("varEnv")=='prod'){
                    ...
                    cy.visit(Cypress.env("prod")+data.exam_overview)
                }else{
                    cy.log(Cypress.env("varEnv"))
                    cy.visit(Cypress.env("stage")+data.exam_overview)
                }
            })
        })
        ...
        it('Btech- Must see articles', () =>{
            cy.fixture('xpath').then((data)=>{
                cy.get(data.exam_mustseenews_btn).click()
                cy.LeadForm('web_india_Btech_MBBSpage')
            })
        })
        ...
        it('Btech- News like button', () =>{
            cy.fixture('xpath').then((data)=>{
                cy.get(data.exam_newslike2_btn).click()
                cy.LeadForm('web_india_Btech_MBBSpage')
            })
        })
    })