    describe('College News Page- Sanity', () => {
        beforeEach(() => {
            ...
            cy.fixture('endpoint').then((data)=>{
                if(Cypress.env("varEnv")=='prod'){
                    ...
                    cy.visit(Cypress.env("prod")+data.college_news)
                }else{
                    cy.log(Cypress.env("varEnv"))
                    cy.visit(Cypress.env("stage")+data.college_news)
                }
            })
        })
        ...
        it('Btech- University news and article', () =>{
            cy.fixture('xpath').then((data)=>{
                cy.get(data.col_uninews_article).click()
                cy.LeadForm('web_india_btech')
            })
        })
