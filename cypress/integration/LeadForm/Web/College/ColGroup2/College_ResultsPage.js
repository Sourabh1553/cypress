    describe('College Results Page- Sanity', () => {
        beforeEach(() => {
            ...
            cy.fixture('endpoint').then((data)=>{
                if(Cypress.env("varEnv")=='prod'){
                    ...
                    cy.visit(Cypress.env("prod")+data.college_result)
                }else{
                    cy.log(Cypress.env("varEnv"))
                    cy.visit(Cypress.env("stage")+data.college_result)
                }
            })
        })
        ...
        it('Btech Form- Related news article', () => {
            cy.fixture('xpath').then((data)=>{
                cy.waitUntil(() => cy.get(data.col_relatednews).then($el => $el.trigger('click')))
                cy.LeadForm('web_india_btech')
            })
        })
        
        // it('Btech Form- Trending news article', () => {
        //     cy.fixture('xpath').then((data)=>{
        //         cy.waitUntil(() => cy.get(data.col_trendingnews).then($el => $el.trigger('click')))
        //         cy.LeadForm('web_india_btech')
        //     })
        // })
