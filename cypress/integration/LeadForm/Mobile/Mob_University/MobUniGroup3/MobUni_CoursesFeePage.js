    describe('Mobile- University CourseFee Page- Sanity', () => {
        beforeEach(() => {
            ...
            cy.fixture('endpoint').then((data)=>{
                if(Cypress.env("varEnv")=='prod'){
                    ...
                    cy.visit(Cypress.env("prod")+data.university_courseFee)
                }else{
                    cy.log(Cypress.env("varEnv"))
                    cy.visit(Cypress.env("stage")+data.university_courseFee)
                }
            })
            cy.intercept('POST')
            cy.intercept('GET')
        ...
        it('Check Fees - Btech Form', () =>{
            cy.waitUntil(() => cy.get(':nth-child(1) > .course-heading > .fees > .text-primary').then($el => $el.trigger('click')))
            cy.LeadForm('mob_india_Btech')
        })

        it('Download Brochure - Course - Btech Form', () =>{
            cy.waitUntil(() => cy.get(':nth-child(1) > .links > .text-success').then($el => $el.trigger('click')))
            cy.LeadForm('mob_india_Btech')
        })
