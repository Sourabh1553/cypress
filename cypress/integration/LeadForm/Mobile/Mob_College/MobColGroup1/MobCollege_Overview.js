    describe('Mobile- College Overview Page- Sanity', () => {
        beforeEach(() => {
            ...
            cy.fixture('endpoint').then((data)=>{
                if(Cypress.env("varEnv")=='prod'){
                    ...
                    cy.visit(Cypress.env("prod")+data.college_info)
                    cy.log(Cypress.env("prod")+data.college_info);
                }else{
                    cy.log(Cypress.env("varEnv"))
                    cy.visit(Cypress.env("stage")+data.college_info)
                    cy.log(Cypress.env("prod")+data.college_info);
                }
            })
            cy.intercept('POST')
        ...
        it('Check Fees - Btech Form', () =>{
            cy.fixture('xpath').then((data)=>{
                cy.log('Launching leadform');
                cy.get(data.mob_check_fees_btn).click()
                cy.log('Filling leadform');
                cy.LeadForm('mob_india_Btech')
                cy.log('Leadform filled successfully');
            })
        ...
        it('Download Brochure - Course - Btech Form', () =>{
            cy.fixture('xpath').then((data)=>{
                cy.log('Launching leadform');
                cy.get(data.mob_db_course_btn).click()
                cy.log('Filling leadform');
                cy.LeadForm('mob_india_Btech')
                cy.log('Leadform filled successfully');
            })
