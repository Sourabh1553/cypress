    describe('Mobile- College Gallery Page- Sanity', () => {
        beforeEach(() => {
            ...
            cy.fixture('endpoint').then((data)=>{
                if(Cypress.env("varEnv")=='prod'){
                    ...
                    cy.visit(Cypress.env("prod")+data.college_gallery)
                    cy.log(Cypress.env("prod")+data.college_gallery);
                }else{
                    cy.log(Cypress.env("varEnv"))
                    cy.visit(Cypress.env("stage")+data.college_gallery)
                    cy.log(Cypress.env("prod")+data.college_gallery);
                }
            })
            cy.intercept('POST')
