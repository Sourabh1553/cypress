    describe('Mobile- College Hostel Page- Sanity', () => {
        beforeEach(() => {
            ...
            cy.fixture('endpoint').then((data)=>{
                if(Cypress.env("varEnv")=='prod'){
                    ...
                    cy.visit(Cypress.env("prod")+data.college_hostel)
                }else{
                    cy.log(Cypress.env("varEnv"))
                    cy.visit(Cypress.env("stage")+data.college_hostel)
                }
            })
            cy.intercept('POST')
            cy.intercept('GET')
        ...
        it('Similar college- Download Brochure - Btech Form', () =>{
            cy.fixture('xpath').then((data)=>{
                cy.wait(1000)
                cy.waitUntil(() => cy.get(data.mob_similar_btn).eq(0).scrollIntoView().then($el => $el.trigger('click')))
                cy.wait(1000)
                cy.LeadForm('mob_india_Btech')
            })
        })
        ...
        it('Nearby college- Download Brochure - Btech Form', () =>{
            cy.fixture('xpath').then((data)=>{
                cy.wait(1000)
                cy.waitUntil(() => cy.get(data.mob_nearby_btn).eq(0).scrollIntoView().then($el => $el.trigger('click')))
                cy.wait(1000)
                cy.LeadForm('mob_india_Btech')
            })
        })
