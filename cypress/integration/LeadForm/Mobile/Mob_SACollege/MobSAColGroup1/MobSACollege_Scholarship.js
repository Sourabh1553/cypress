    describe('Mobile- Study Abroad College Scholarship Page- Sanity', () => {
        beforeEach(() => {
            ...
            cy.fixture('endpoint').then((data)=>{
                if(Cypress.env("varEnv")=='prod'){
                    ...
                    cy.visit(Cypress.env("prod")+data.SA_scholarship)
                }else{
                    cy.log(Cypress.env("varEnv"))
                    cy.visit(Cypress.env("stage")+data.SA_scholarship)
                }
            })
            cy.intercept('POST')
            cy.intercept('GET')
