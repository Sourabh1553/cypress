    describe('Mob College Admission Page- Sanity', () => {
        beforeEach(() => {
            ...
            cy.fixture('endpoint').then((data)=>{
                if(Cypress.env("varEnv")=='prod'){
                    ...
                    cy.visit(Cypress.env("prod")+data.admissionPage)
                }else{
                    cy.log(Cypress.env("varEnv"))
                    cy.visit(Cypress.env("stage")+data.admissionPage)
                }
            })
        })
        ...
        it('Follow button- Footer - Btech Form', () =>{
            cy.get('.jsx-3428355176').click()
            cy.LeadForm('mob_india_Btech')
        })
